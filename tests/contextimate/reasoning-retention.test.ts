...
test("a replay signature alone does not prove historical retention", () => {
  const unmeasured = SessionManager.inMemory("/tmp/contextimate-unmeasured-thinking");
  unmeasured.appendMessage(assistantMessage(
  ...
  unmeasured.appendMessage({ role: "user", content: "next", timestamp: 2 });
  unmeasured.appendMessage(assistantMessage(
    [{ type: "text", text: "done" }],
    { api: "google-generative-ai", provider: "google", model: "gemini-3-pro", usage: usageWithReasoning(50) },
  ));
  assert.equal(buildSessionBreakdown(unmeasured)!.reasoningTokens, 50);
});
