...
test("cachemire ledger and one-line surfaces golden", () => {
  ...
  const lines = [
    ...
    clock({ now: 1 * MIN, ...base, modelSwitched: true, switchForecast: { targetId: "gpt-5.6-luna", targetProvider: "openai-codex", estTokens: 96_400, basis: "direct" } }),
    clock({ now: 1 * MIN, ...base, modelSwitched: true, switchForecast: { targetId: "gpt-5.6-luna", targetProvider: "openai-codex", estTokens: 96_400, basis: "direct", breakdown: { anchorTokens: 80_100, droppedThinking: 2_100 } } }),
    clock({ now: 1 * MIN, ...base, modelSwitched: true, switchForecast: { targetId: "gpt-5.6-luna", targetProvider: "radius", estTokens: 96_400, basis: "gateway" } }),
    ...
    `\u25cd ${renderBreakingLine({ cause: { kind: "model", detail: "model switched claude-opus-4-8 \u2192 gpt-5.6-luna" }, estimatedRewriteTokens: 96_400, targetProvider: "openai-codex", estimateBreakdown: { anchorTokens: 80_100, droppedThinking: 2_100 }, estimatedUsd: 0.12, estimateBasis: "direct" })}`,
