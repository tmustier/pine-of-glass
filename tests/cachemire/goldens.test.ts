// Visual regression net for cachemire's user-facing strings (design language §11):
...
test("cachemire ledger and one-line surfaces golden", () => {
  ...
  const lines = [
    ...
    clock({ now: 1 * MIN, ...base, modelSwitched: true, switchForecast: { targetId: "gpt-5.6-luna", targetProvider: "openai-codex", windowTokens: 272_000, estTokens: 96_400, basis: "direct", droppedReasoningChars: 0 } }),
    clock({ now: 1 * MIN, ...base, modelSwitched: true, switchForecast: { targetId: "gpt-5.6-luna", targetProvider: "radius", windowTokens: 272_000, estTokens: 96_400, basis: "gateway", droppedReasoningChars: 0 } }),
    clock({ now: 1 * MIN, ...base, modelSwitched: true, switchForecast: { targetId: "claude-opus-4-8", targetProvider: "anthropic", windowTokens: 200_000, estTokens: 131_000, basis: "direct", droppedReasoningChars: 0, prior: { requestAt: 0, window: CONTRACT_5M } } }),
    ...
    `\u25cd ${renderBreakingLine({ cause: { kind: "model", detail: "model switched claude-opus-4-8 \u2192 gpt-5.6-luna" }, estimatedRewriteTokens: 96_400, targetWindowTokens: 272_000, estimatedUsd: 0.12, estimateBasis: "direct" })}`,
