...
test("cachemire ledger and one-line surfaces golden", () => {
  ...
  const lines = [
    ...
    clock({ now: 1 * MIN, ...base, modelSwitched: true, switchForecast: { targetId: "gpt-5.6-luna", targetProvider: "openai-codex", windowTokens: 272_000, estTokens: 96_400, basis: "direct" } }),
    clock({ now: 1 * MIN, ...base, modelSwitched: true, switchForecast: { targetId: "gpt-5.6-luna", targetProvider: "radius", windowTokens: 272_000, estTokens: 96_400, basis: "gateway" } }),
    clock({ now: 1 * MIN, ...base, modelSwitched: true, switchForecast: { targetId: "claude-opus-4-8", targetProvider: "anthropic", windowTokens: 200_000, estTokens: 131_000, basis: "direct", prior: { requestAt: 0, window: CONTRACT_5M } } }),
