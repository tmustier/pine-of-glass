...
- **Paginated reads**: consecutive reads of the same file (paging past the read cap)
  ...
  visible between the reads breaks the fold, and Ctrl+T's native view restores the
  individual rows.
...
- Plain terminal scrolling is never touched: traceline requests no mouse reporting.

The collapse state is read from a live assistant row, so the tool view can never desync from the reasoning view.
