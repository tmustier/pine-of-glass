...
- **How much a file mutation changed** - diff-backed `edit` and live-snapshotted `write` rows include `+N -M` before the size suffix, with zero sides dropped (`edit … +3 -1 · 0.1k ch`, `write … +5`). Within a block the diff cells form two sign-aligned columns: the widest `+`/`-` tokens set the column widths and a dropped side holds its column as blank space, so a plus-only `+18` and a minus-only `-1` line up under each other instead of both crowding the `·` — every `+`, every `-`, and the separator each keep one x down the block. `write` stats come from a pre-execution snapshot, so historical/restored rows that were not seen live may still show only the size suffix.
