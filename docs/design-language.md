...
reasoning gets an exact `Reasoning output` row for the response anchoring Pi's total,
including a reported zero. Replayed provider-generated summaries not covered by that
exact count get a separate estimated `Thinking summaries` row; opaque signatures are
