...
OpenAI's [reasoning context policy](https://developers.openai.com/api/docs/guides/reasoning#preserve-reasoning-across-calls) defaults GPT-5.6 and later models, including GPT-6 Astra, to `all_turns`. Earlier models default to `current_turn`. Controlled Codex tests confirmed that the service removes earlier-turn reasoning before building the context for GPT-5.5, even when Pi resends the encrypted items. A sanitized two-turn Astra Codex probe confirmed that Pi preserved the encrypted carrier and reported reasoning usage, but Pi does not expose the effective `reasoning.context` request value. The probe therefore did not conclusively verify `all_turns` live. Contextimate follows the documented default for encrypted Responses and Codex reasoning items and bounds historical attribution by the exact prompt total. An ordinary OpenAI text item id is not a reasoning carrier. Direct OpenAI API-key and Azure Responses tests remain pending.
