...
After a model switch, Pi's exact count is still denominated in the *previous* model's tokenizer while the window belongs to the new one; composing the two would be fiction. The panel names the currency instead: `Total request` keeps the exact count but its detail becomes `(pre-switch usage · gpt-5.6-sol tokens)`, the window share and context bar are withheld, and the session split falls back to the heuristic rather than anchoring the residual to an old-currency exact. The first post-switch response re-baselines everything.
