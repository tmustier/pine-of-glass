...
OpenAI's [reasoning context policy](https://platform.openai.com/docs/guides/reasoning#preserve-reasoning-across-calls) defaults GPT-5.6 models to `all_turns`. Earlier models default to `current_turn`. Contextimate follows those defaults for encrypted Responses and Codex reasoning items. An ordinary OpenAI text item id is not a reasoning carrier.
