...
OpenAI's [reasoning context policy](https://platform.openai.com/docs/guides/reasoning#preserve-reasoning-across-calls) defaults GPT-5.6 models to `all_turns`. Earlier models default to `current_turn`. Older encrypted items can remain in the API payload and its cached prefix under `current_turn`; the service does not render them into the next sample. A cache hit therefore does not prove that historical reasoning occupies model context. Contextimate follows the effective default for encrypted Responses and Codex reasoning items. An ordinary OpenAI text item id is not a reasoning carrier.
