...
OpenAI's [reasoning context policy](https://platform.openai.com/docs/guides/reasoning#preserve-reasoning-across-calls) defaults GPT-5.6 models to `all_turns`. Earlier models default to `current_turn`, although GPT-5.5 accepts an explicit `all_turns`. OpenAI's own Codex client resolves the accounting ambiguity. It [replays the complete encrypted history](https://github.com/openai/codex/blob/8922a784fe6aa80683fe97c2dcdfdc361478aa7f/codex-rs/core/src/client_common.rs#L51-L59), leaves standard Responses on the default `current_turn`, and uses `all_turns` for Responses Lite. More importantly, Codex [adds reasoning items before the latest user boundary](https://github.com/openai/codex/blob/8922a784fe6aa80683fe97c2dcdfdc361478aa7f/codex-rs/core/src/context_manager/history.rs#L263-L315) because, as [OpenAI's implementing PR states](https://github.com/openai/codex/pull/7113), the API total does not account for them. A server `x-reasoning-included` header tells Codex not to add them twice.

Contextimate follows that client behavior but uses each item's original exact `usage.reasoning` rather than Codex's encrypted-length estimate. This is active-request and compaction accounting; it does not claim that `current_turn` renders older reasoning into the next sample. Direct two-turn probes on 3 August 2026 made the fallback boundary reproducible: GPT-5.6 reported a next-prompt increase covering the prior 178 reasoning tokens, while GPT-5.5 with explicit `all_turns` increased reported input by only 8 after generating 516 reasoning tokens. Pi does not expose Codex's per-response inclusion header, so Contextimate uses the measured GPT-5.6 model boundary. Azure Responses follows the same API handling in code but has not been probed separately. A cache hit alone still does not reveal which path applies. An ordinary OpenAI text item id is not a reasoning carrier.
